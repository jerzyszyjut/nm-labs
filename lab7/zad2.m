...
function [integration_error, Nt, ft_5, integral_1000] = zadanie2()
    ...
    ft_5 = f(5);
    integral_1000 = trapez_method_integral(f, 0, 5, 1000);

    for i = 1:length(Nt)
        integration_result = trapez_method_integral(f, 0, 5, Nt(i));
...
function result = trapez_method_integral(f, a, b, n)
    h = (b - a) / n;
    x = a:h:b;
    result = 0;

    for i = 1:n
        result = result + (f(x(i)) + f(x(i+1))) / 2;
    end
